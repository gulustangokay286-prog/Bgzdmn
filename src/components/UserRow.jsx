...
const UserRow = ({ document, showApprovalActions, onUpdate }) => {
  ...
  const getRoleBadge = (r) => {
    switch(r.toLowerCase()) {
      case 'student':
      case 'öğrenci': return <span className="badge bg-blue-100 text-blue-700">Öğrenci</span>;
      case 'teacher':
      case 'öğretmen': return <span className="badge bg-purple-100 text-purple-700">Öğretmen</span>;
      case 'parent':
      case 'veli': return <span className="badge bg-emerald-100 text-emerald-700">Veli</span>;
      case 'personnel':
      case 'personel': return <span className="badge bg-teal-100 text-teal-700">Personel</span>;
      case 'admin':
      case 'yönetici': return <span className="badge bg-amber-100 text-amber-700">Yönetici</span>;
      default: return <span className="badge bg-slate-50 dark:bg-[#1e293b] text-slate-700 dark:text-slate-300">{r}</span>;
    }
  };
  ...
  return (
    <>
      <div className="flex items-center py-4 text-sm relative">
        ...
        <div style={{ width: '20%' }} className="text-slate-500 truncate pr-2" title={email}>
          {email}
        </div>

        <div style={{ width: '15%' }} className="pr-2">
          {status === 'approved' ? (
            <span className="flex items-center gap-1.5 text-emerald-600 font-semibold text-xs"><CheckCircle size={14}/> Onaylı</span>
          ) : status === 'rejected' ? (
            <span className="flex items-center gap-1.5 text-rose-600 font-semibold text-xs"><XCircle size={14}/> Reddedildi</span>
          ) : (
            <span className="flex items-center gap-1.5 text-amber-500 font-semibold text-xs animate-pulse"><Info size={14}/> Bekliyor</span>
          )}
        </div>
        ...
        <div className="flex-1 flex justify-end gap-2 items-center">
          <button 
            ...
            className="p-1.5 rounded-lg text-slate-600 dark:text-slate-400 hover:text-[#0f172a] hover:bg-slate-50 dark:bg-[#1e293b] transition-colors border border-transparent hover:border-slate-200 dark:border-white/10"
            title="Detayları Gör"
          >
            <Search size={18} />
          </button>
          <button 
            ...
            className={`p-1.5 rounded-lg transition-colors border border-transparent ${
              isAdmin 
                ? 'text-slate-700 dark:text-slate-300 cursor-not-allowed opacity-60' 
                : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-[#991b1b] hover:border-[#991b1b]'
            }`}
            ...
            isProcessing ? (
              ...
              <div className="flex gap-1 relative">
                <button 
                  ...
                  className="p-1.5 rounded-lg text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-white dark:bg-[#0f172a] transition-colors border border-transparent hover:border-[#0f172a]"
                  title="Onayla"
                >
                  <CheckCircle size={18} />
                </button>
                <button 
                  ...
                  className="p-1.5 rounded-lg text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-[#991b1b] transition-colors border border-transparent hover:border-[#991b1b]"
                  title="Reddet"
                >
                  <XCircle size={18} />
                </button>
                ...
                {showAssignClass && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-[#0f172a] rounded-xl shadow-xl border border-slate-200 dark:border-white/10 p-4 z-50 animate-fade-in">
                    <div className="flex justify-between items-center mb-3">
                      <h4 className="text-sm font-bold text-navy m-0">Şube Ataması</h4>
                      <button onClick={() => setShowAssignClass(false)} className="text-slate-600 dark:text-slate-400 hover:text-red-500"><X size={16}/></button>
                    </div>
                    <div className="mb-3">
                      <label className="text-xs font-semibold text-slate-500 uppercase mb-1.5 block">Sınıf Şubesi</label>
                      <select 
                        className="w-full bg-slate-50 dark:bg-[#1e293b] border border-slate-200 dark:border-white/10 rounded-lg p-2 text-sm outline-none focus:border-info"
                    ...
                    <button onClick={handleApproveWithBranch} className="w-full bg-white dark:bg-[#0f172a] hover:bg-[#1e3a8a] text-slate-900 dark:text-white font-semibold text-sm py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5">
      ...
      {showDetails && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative w-full max-w-md bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/10/60 rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-300">
            
            <button 
              className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-50 dark:bg-[#1e293b] text-slate-600 dark:text-slate-400 hover:text-slate-600 dark:text-slate-400 transition-colors z-10"
            ...
            <div className="p-6">
                ...
                <div className="relative">
                  ...
                  {isAdmin && (
                    <div className="absolute -bottom-1 -right-1 bg-white dark:bg-[#0f172a] rounded-full p-0.5 shadow-sm">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="#3b82f6" className="text-slate-900 dark:text-white" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="m9 12 2 2 4-4" stroke="white" strokeWidth="3"/></svg>
                    </div>
                  )}
                </div>
                
                <div>
                  <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200 tracking-tight leading-none mb-1.5">{name}</h2>
                  <div className="flex items-center gap-2">
                    {getRoleBadge(roleRaw)}
                    {status === 'approved' && <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Onaylı</span>}
                    {status === 'pending' && <span className="text-[10px] font-bold text-amber-600 uppercase tracking-wider">Bekliyor</span>}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                ...
                <div className="flex flex-col p-3 rounded-xl bg-slate-50 dark:bg-[#1e293b] border border-slate-200 dark:border-white/10">
                  ...
                  <div className="text-[13px] font-semibold text-slate-700 dark:text-slate-300 truncate">{tc}</div>
                </div>
                ...
                <div className="flex flex-col p-3 rounded-xl bg-slate-50 dark:bg-[#1e293b] border border-slate-200 dark:border-white/10">
                  ...
                  <div className="text-[13px] font-semibold text-slate-700 dark:text-slate-300 truncate">{document.fields?.phone?.stringValue || '-'}</div>
                </div>
                ...
                <div className="flex flex-col p-3 rounded-xl bg-slate-50 dark:bg-[#1e293b] border border-slate-200 dark:border-white/10 sm:col-span-2">
                  ...
                  <div className="text-[13px] font-semibold text-slate-700 dark:text-slate-300 truncate">{document.fields?.email?.stringValue || '-'}</div>
                </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-white/10">
                ...
                <button 
                  onClick={() => setShowDetails(false)} 
                  className="bg-slate-900 hover:bg-slate-800 text-slate-900 dark:text-white px-5 py-2 rounded-lg text-[13px] font-medium transition-colors"
                >
                  Kapat
                </button>
              </div>
