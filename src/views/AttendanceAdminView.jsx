...
const AttendanceAdminView = () => {
  ...
  // 3. Devamsızlık Özeti — ortak kural motoruyla hesaplanır.
  ...
  const attendanceSummary = useMemo(() => {
    ...
    // Aynı güne ait kayıtlar birlikte değerlendirilir; bir gün 1.0'ı aşamaz.
    const byDay = {};
    ...
    Object.values(byDay).forEach(dayRecords => {
      const excused = dayRecords.filter(r =>
        r.status === 'excused' ||
      ...
      if (excused.length) {
        ...
        const weight = excused.some(r =>
          Number(r.absenceWeight) === 0.5 || String(r.courseName || '').includes('Yarım Gün')) ? 0.5 : 1.0;
        raporluTotal += weight;
        return;
      }
    ...
    // Karekod geçişlerindeki geç kalmalar
    qrLogs.forEach(l => { if (l.isLate) lateCount++; });
  ...
  // 3b. Öğrencinin BUGÜNKÜ canlı durumu (kural motoru ile)
            ...
            <div className="flex-1 overflow-y-auto pb-16">
              ...
              <div className="px-6 md:px-10 py-6 border-b border-slate-200 dark:border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/40 dark:bg-slate-900/40">
                ...
                <div className="flex gap-3">
                  ...
                  <div className="px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-white/10 flex flex-col items-center min-w-[85px]">
                    <span className="text-[20px] font-extrabold text-slate-700 dark:text-slate-300 leading-none">
                      {attendanceSummary.autoCount}
                    </span>
                    <span className="text-[10.5px] font-bold text-slate-500 dark:text-slate-400 uppercase mt-1">Otomatik</span>
                  </div>
                </div>
              </div>


              {/* BUGÜNKÜ CANLI DURUM — kural motorunun anlık kararı */}
              {viewMode === 'student' && todayEvaluation && (
                <div className="px-6 md:px-10 py-5 border-b border-slate-100 dark:border-white/5 bg-slate-50/60 dark:bg-slate-900/30">
                  <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                    <span className="text-[12px] font-bold text-slate-400 uppercase tracking-wider">Bugünkü Durum (Canlı)</span>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {/* Sabah oturumu */}
                    <div className={`flex-1 min-w-[190px] px-4 py-3 rounded-xl border ${
                      todayEvaluation.morning.present
                        ? 'bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-900/50'
                        : todayEvaluation.morning.finalized
                          ? 'bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-900/50'
                          : 'bg-white dark:bg-[#0f172a] border-slate-200 dark:border-white/10'
                    }`}>
                      <span className="text-[10.5px] font-bold uppercase tracking-wider text-slate-400 block">Sabah Oturumu</span>
                      <span className="text-[14px] font-extrabold text-slate-900 dark:text-white">
                        {todayEvaluation.morning.present
                          ? `Var · ${todayEvaluation.morning.entryTime}${todayEvaluation.morning.isLate ? ' (geç)' : ''}`
                          : todayEvaluation.morning.finalized ? 'Yok (0,5 gün)' : 'Bekleniyor'}
                      </span>
                    </div>

                    {/* Öğleden sonra oturumu */}
                    <div className={`flex-1 min-w-[190px] px-4 py-3 rounded-xl border ${
                      todayEvaluation.afternoon.present
                        ? 'bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-900/50'
                        : todayEvaluation.afternoon.finalized
                          ? 'bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-900/50'
                          : 'bg-white dark:bg-[#0f172a] border-slate-200 dark:border-white/10'
                    }`}>
                      <span className="text-[10.5px] font-bold uppercase tracking-wider text-slate-400 block">Öğleden Sonra</span>
                      <span className="text-[14px] font-extrabold text-slate-900 dark:text-white">
                        {todayEvaluation.afternoon.present
                          ? `Var · ${todayEvaluation.afternoon.entryTime}${todayEvaluation.afternoon.isLate ? ' (geç)' : ''}`
                          : todayEvaluation.afternoon.finalized ? 'Yok (0,5 gün)' : 'Bekleniyor'}
                      </span>
                    </div>

                    {/* Günün sonucu */}
                    <div className="flex-1 min-w-[190px] px-4 py-3 rounded-xl border bg-white dark:bg-[#0f172a] border-slate-200 dark:border-white/10">
                      <span className="text-[10.5px] font-bold uppercase tracking-wider text-slate-400 block">Bugünün Sonucu</span>
                      <span className="text-[14px] font-extrabold text-slate-900 dark:text-white">
                    ...
                    {/* Konum */}
                    <div className="flex-1 min-w-[160px] px-4 py-3 rounded-xl border bg-white dark:bg-[#0f172a] border-slate-200 dark:border-white/10">
                      <span className="text-[10.5px] font-bold uppercase tracking-wider text-slate-400 block">Konum</span>
                      <span className="text-[14px] font-extrabold text-slate-900 dark:text-white flex items-center gap-1.5">
                        {todayEvaluation.isInside ? <DoorClosed size={15} className="text-emerald-500" /> : <DoorOpen size={15} className="text-amber-500" />}
                        {todayEvaluation.isInside ? 'Kurum İçinde' : 'Kurum Dışında'}
                      </span>
                    </div>
                  </div>

                  {(todayEvaluation.needsAutoLunchExit || todayEvaluation.needsAutoSchoolExit) && (
                    <div className="mt-3 text-[12.5px] font-semibold text-amber-700 dark:text-amber-300 flex items-center gap-2">
                      <Timer size={14} />
                      Otomatik çıkış sırada: sistem bir sonraki turda çıkışını verecek.
                    </div>
                  )}
                </div>
              )}
                      ...
                      return (
                        <div key={record.id} className="flex items-center justify-between p-3.5 rounded-xl border border-slate-200 dark:border-white/10 hover:border-slate-300 bg-white dark:bg-[#0f172a] shadow-xs transition-all">
                          <div className="flex items-center gap-3.5">
                            ...
                            <div>
                              <div className="font-bold text-slate-900 dark:text-white text-[13.5px] flex items-center gap-2 flex-wrap">
                                {cName}
                                {record.autoGenerated && (
                                  <span className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[9.5px] font-extrabold uppercase tracking-wider border border-slate-200 dark:border-white/10">
                                    Otomatik
                                  </span>
                                )}
                              </div>
                              <div className="text-[11.5px] text-slate-500 font-medium mt-0.5">
                                Tarih: {rDate} • Yetkili: {record.recordedBy || 'Admin'}
