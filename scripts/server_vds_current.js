...
app.get('/api/attendance/live', async (req, res) => {
    try {
        ...
        // Format and deduplicate logs within 5 seconds for the same student and action
        const deduped = [];
        const seen = new Set();
        for (const l of logs) {
            const timeSec = Math.floor(new Date(l.timestamp).getTime() / 5000);
            const key = `${l.studentId}_${l.action || l.status}_${timeSec}`;
            if (seen.has(key)) continue;
            seen.add(key);
            deduped.push(l);
        }

        const formatted = deduped.map(l => ({
            id: String(l._id),
